...
<p align="center">
  <a href="https://github.com/dgtpizza/taptap/blob/main/screenshots/video.mp4">▶️ Демо-видео</a>
</p>
...
- **Грубый IP-guard для `/api/*`.** Перед авторизацией стоит общий лимит по IP, чтобы дешево резать
мусорный поток, брутфорс подписи и неавторизованный спам. Для локальных нагрузочных тестов его можно
отключить через `API_IP_RATE_LIMIT_MS=0`, потому что все виртуальные пользователи идут с одного
localhost.
...
бесконечный backlog, а максимум одну серверную пачку. Подменённый `pending: 999999` не разгонит даже
...
| Грубый лимит `/api/*` по IP       | 1 запрос в 50 мс на IP           |
...
| Повтор той же пачки при ретрае | `nonce` делает повтор идемпотентным; concurrent replay покрыт integration-тестом. |
| Частый спам `/api/clicks` | Персональный Mongo-лимитер возвращает 429, а энергия не даёт набрать больше нормы. |
| Неавторизованный поток или брутфорс подписи | `/api/*` режется грубым IP-guard ещё до проверки Telegram `initData`. |
| Подмена `localStorage` с огромной очередью | Клиент восстановит максимум одну серверную пачку, затем сверится с backend. |
...
Для быстрой проверки `/api/clicks` есть воспроизводимый load-test без внешних сервисов: он поднимает
MongoDB через Testcontainers, запускает backend на случайном порту и подписывает `initData` тестовым
токеном.

```bash
LOAD_AUTOSTART=1 LOAD_USERS=50 LOAD_SECONDS=10 LOAD_INTERVAL_MS=450 npm run load:api
```

Последний локальный прогон на MacBook:

```text
requests=1150 ok=1150 networkErrors=0 rps=105.5
acceptedClicks=1150 statuses=200:1150
latency p50=38.8 ms p95=79.0 ms p99=215.0 ms max=226.2 ms
```

Для прогона по уже запущенному API:

```bash
BOT_TOKEN=<token> LOAD_BASE_URL=http://127.0.0.1:3000/api npm run load:api
```

Если все виртуальные пользователи идут с одного localhost, грубый IP-guard можно отключить на backend:
`API_IP_RATE_LIMIT_MS=0`. Пользовательские лимиты и энергия при этом остаются включены.
...
| `API_IP_RATE_LIMIT_MS` | грубый лимит `/api/*` по IP, по умолчанию `50`, `0` отключает |
