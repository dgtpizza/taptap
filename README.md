...
Для проверки `/api/clicks` под потоком есть воспроизводимый тест без внешних сервисов: он поднимает
MongoDB через Testcontainers, запускает бэкенд на случайном порту и подписывает `initData` тестовым
токеном. Это не поиск абсолютного потолка RPS, а проверка, что заданный поток проходит через
реальные Telegram-auth, Fastify, MongoDB, атомарный update, nonce и энергию без ошибок и деградации.

```bash
LOAD_AUTOSTART=1 LOAD_USERS=50 LOAD_SECONDS=10 LOAD_INTERVAL_MS=450 npm run test:load
```

Этот профиль даёт примерно 111 запросов в секунду. Последний локальный прогон на MacBook:
...
Отдельно можно замерить пропускную способность связки Fastify + MongoDB, отделив её от бизнес-лимита
кликов. Для этого пользовательский лимит записи отключается намеренно, это не продовый режим:
...
Цифры привязаны к локальной машине, Docker/Testcontainers и MongoDB без тюнинга. Они показывают
порядок и отсутствие очевидного узкого места в коде, но это не SLA для продовой инфраструктуры.
