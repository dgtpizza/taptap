...
<p align="center">
  <img src="https://raw.githubusercontent.com/dgtpizza/taptap/main/screenshots/video.gif" alt="Демо" width="16%" />
  <img src="https://raw.githubusercontent.com/dgtpizza/taptap/main/screenshots/clicker.png" alt="Кликер" width="16%" />
  <img src="https://raw.githubusercontent.com/dgtpizza/taptap/main/screenshots/leaderboard.png" alt="Лидерборд" width="16%" />
  <img src="https://raw.githubusercontent.com/dgtpizza/taptap/main/screenshots/network.png" alt="Сетевая ошибка" width="16%" />
  <img src="https://raw.githubusercontent.com/dgtpizza/taptap/main/screenshots/unauthorized.png" alt="Сессия истекла" width="16%" />
  <img src="https://raw.githubusercontent.com/dgtpizza/taptap/main/screenshots/notfound.png" alt="404" width="16%" />
</p>
...
### InitData на каждом запросе вместо JWT

Клиент отправляет `Authorization: tma <initData>` на каждый API-запрос, как в официальном паттерне
Telegram Mini Apps. Сервер каждый раз проверяет HMAC-подпись и срок жизни `auth_date`, поэтому не
нужно выпускать собственный JWT, хранить refresh-токены или решать revocation для двух API-экранов.
HMAC-проверка дешёвая; узкое место здесь скорее MongoDB и бизнес-логика, а не подпись Telegram.
Если бы приложение получило non-Telegram клиентов, роли, админку или долгоживущие сессии, тогда
`/auth/telegram -> access/refresh JWT` стал бы оправданным следующим шагом.
...
- **Недоверенная локальная очередь.** После перезагрузки клиент восстанавливает из `localStorage` не
бесконечный хвост, а максимум одну серверную пачку. Подменённый `pending: 999999` не разгонит даже
оптимистичный счёт; сервер всё равно остаётся источником истины.
...
| --- | --- |
| Клиент пытается прислать итоговый `clicks` | Такого поля в контракте нет; сервер принимает только дельту и сам считает итог. |
| Большая пачка кликов | `count > MAX_BATCH` отклоняется схемой, энергия всё равно ограничивает темп. |
| Повтор той же пачки при ретрае | `nonce` делает повтор идемпотентным; одновременный повтор покрыт integration-тестом. |
| Частый спам `/api/clicks` | Персональный Mongo-лимитер возвращает 429, а энергия не даёт набрать больше нормы. |
| Неавторизованный поток или брутфорс подписи | Подпись `initData` проверяется HMAC в постоянном времени и отклоняется с 401 ещё до работы с базой. |
| Подмена `localStorage` с огромной очередью | Клиент восстановит максимум одну серверную пачку, затем сверится с сервером. |
| Бот собирает энергию 24/7 | Полностью не решается в рамках простого тестового; на проде нужны поведенческие эвристики и суточные лимиты. |
...
Пять уровней, всего **185 тестов, все зелёные**:

| Уровень | Что покрывает | Тестов |
| --- | --- | --- |
| Backend unit (Vitest) | формула энергии, валидация `initData` | 35 |
| Backend integration (Vitest + Testcontainers) | лидерборд, идемпотентность, рейтлимитер, реген на реальной Mongo | 27 |
...
Для проверки `/api/clicks` под нагрузкой есть воспроизводимый тест без внешних сервисов: он поднимает
MongoDB через Testcontainers, запускает бэкенд на случайном порту и подписывает `initData` тестовым
токеном. Это не замер потолка RPS, а проверка, что заданная нагрузка проходит весь настоящий путь
запроса (проверка Telegram `initData`, Fastify, атомарное обновление в MongoDB, защита от повторов по
`nonce`, энергия) без ошибок и деградации.

```bash
LOAD_AUTOSTART=1 LOAD_USERS=50 LOAD_SECONDS=10 LOAD_INTERVAL_MS=450 npm run test:load
```

Этот профиль задаёт примерно 111 запросов в секунду. Прогон на MacBook M1, 16 ГБ:

```text
requests=1139 ok=1139 networkErrors=0 rps=109.0
acceptedClicks=1139 statuses=200:1139
latency p50=47.8 ms p95=107.4 ms p99=205.9 ms max=211.4 ms
```

Можно отдельно замерить пропускную способность связки Fastify + MongoDB, без бизнес-лимита кликов.
Для этого пользовательский лимит записи отключается намеренно, это не продовый режим:

```bash
CLICK_RATE_LIMIT_MS=0 LOAD_AUTOSTART=1 LOAD_USERS=100 LOAD_SECONDS=10 LOAD_INTERVAL_MS=100 npm run test:load
```

Тот же MacBook M1, 16 ГБ:

```text
requests=7707 ok=7707 networkErrors=0 rps=763.3
acceptedClicks=7707 statuses=200:7707
latency p50=67.4 ms p95=236.4 ms p99=592.9 ms max=787.7 ms
```

Цифры зависят от конкретной машины, Docker/Testcontainers и MongoDB без тюнинга. Они дают порядок
величин и показывают, что явного узкого места в коде нет, но это не SLA для прода.

Чтобы прогнать по уже запущенному API:

```bash
BOT_TOKEN=<token> LOAD_BASE_URL=http://127.0.0.1:3000/api npm run test:load
```
